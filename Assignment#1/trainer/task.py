...
def learn_rate(params, step):
    return params['lr1'] + tf.compat.v1.train.exponential_decay(params['lr0'], step, params['lr2'], 1/math.e)


# Model loss (not needed in INFER mode)
def conv_model_loss(Ylogits, Y_, mode):
    if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
        loss = tf.reduce_mean(tf.compat.v1.losses.softmax_cross_entropy(tf.one_hot(Y_,10), Ylogits)) * 100
...
def conv_model_train_op(loss, mode, params):
    if mode == tf.estimator.ModeKeys.TRAIN:
        lr = learn_rate(params, tf.compat.v1.train.get_or_create_global_step())
        tf.summary.scalar("learn_rate", lr)
        optimizer = tf.compat.v1.train.AdamOptimizer(lr)
        return slim.training.create_train_op(loss, optimizer)
    else:
        return None
...
def conv_model(features, labels, mode, params):
    ...
    weights_init = tf.compat.v1.truncated_normal_initializer(stddev=0.1)

    def batch_norm_cnv(inputs):
        return tf.compat.v1.layers.batch_normalization(inputs, axis=3, momentum=params['bnexp'], epsilon=1e-5, scale=False, training=(mode == tf.estimator.ModeKeys.TRAIN))

    def batch_norm(inputs):
        return tf.compat.v1.layers.batch_normalization(inputs, axis=1, momentum=params['bnexp'], epsilon=1e-5, scale=False, training=(mode == tf.estimator.ModeKeys.TRAIN))

    XX = tf.reshape(X, [-1, 28, 28, 1])
    Y1 = tf.compat.v1.layers.conv2d(XX,  filters=params['conv1'],  kernel_size=[6, 6], padding="same", kernel_initializer=weights_init)
    Y1bn = tf.nn.relu(batch_norm_cnv(Y1))

    Y2 = tf.compat.v1.layers.conv2d(Y1bn, filters=params['conv2'], kernel_size=[5, 5], padding="same", strides=2, kernel_initializer=weights_init)
    Y2bn = tf.nn.relu(batch_norm_cnv(Y2))

    Y3 = tf.compat.v1.layers.conv2d(Y2bn, filters=params['conv3'], kernel_size=[4, 4], padding="same", strides=2, kernel_initializer=weights_init)
    Y3bn = tf.nn.relu(batch_norm_cnv(Y3))

    Y4 = tf.reshape(Y3bn, [-1, params['conv3']*7*7])
    Y5 = tf.compat.v1.layers.dense(Y4, 200, kernel_initializer=weights_init)
    Y5bn = tf.nn.relu(batch_norm(Y5))


    # to deactivate dropout on the dense layer, set rate=1. The rate is the % of dropped neurons.
    Y5d = tf.compat.v1.layers.dropout(Y5bn, rate=params['dropout'], training=(mode == tf.estimator.ModeKeys.TRAIN))
    Ylogits = tf.compat.v1.layers.dense(Y5d, 10)
